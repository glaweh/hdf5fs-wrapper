...
open(my $header_fh,'>','wrapper_libc_autogenerated_real.h') or die "cannot open 'wrapper_libc_autogenerated_real.h' for writing";
print $header_fh <<"CCODE";
#ifndef WRAPPER_LIBC_AUTOGENERATED_REAL_H
#define WRAPPER_LIBC_AUTOGENERATED_REAL_H

/*
 * This file was automatically generated from $io_calls_template by $0
 */

CCODE

open(my $out_fh,'>','wrapper_libc_autogenerated.c') or die "cannot open 'wrapper_libc_autogenerated.c' for writing";
print $out_fh <<"CCODE";
...
while (<$in_fh>) {
    ...
    if (/^\/\/cmpautowrap:\s+(.+?)\s*$/) {
        push @autowrap,$1 if ((! $noautowrap) and $comparewrap);
        next;
    }
    if (/^\/\/nocmpautowrap:\s+(.+?)\s*$/) {
        push @autowrap,$1 if ((! $noautowrap) and (! $comparewrap));
        next;
    }
    if (/^\/\/need_khiter\s*$/) {
