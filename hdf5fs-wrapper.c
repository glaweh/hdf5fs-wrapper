...
int open64(const char *pathname, int flags, ...) {
    ...
    if ((match_idx=map_filename(pathname,mapped)) >= 0) {
        ...
        fd=__real_open64(mapped,flags,mode);
#else
        // open dummy handle
        fd=__real_open64("/dev/null",O_RDONLY,mode);
#endif
        if (fd < 0) return(fd);
        if (fd > HANDLES_MAX) {
...
//        return(__real___xstat64(version,mapped,buf));
    }
    return(__real___xstat64(version,pathname,buf));
}
...
FILE *fopen(const char *pathname,const char *mode) {
    ...
    if ((match_idx=map_filename(pathname,mapped)) >= 0) {
        ...
        FILE *file;
#ifdef BOTH_HDF_AND_FILE
        file = __real_fopen(mapped,mode);
#else
        file =__real_fopen("/dev/null","r");
#endif
        if (file == NULL)
            return(NULL);
        nfiles++;
...
#ifdef BOTH_HDF_AND_FILE
    __real_fseek(stream,count,SEEK_CUR);
#endif
    return(count);
//    return(__real_fread(ptr,size,nmemb,stream));
}
...
#ifdef BOTH_HDF_AND_FILE
    fwritten=__real_fwrite(ptr,size,nmemb,stream);
#endif
...
#ifdef BOTH_HDF_AND_FILE
    __real_lseek64(fd,rcount,SEEK_CUR);
#endif
    return(rcount);
//    return(__real_read(fd,buf,count));
}
...
#ifdef BOTH_HDF_AND_FILE
    fwritten=__real_write(fd,buf,count);
#endif
...
#ifdef BOTH_HDF_AND_FILE
    int foff = __real_lseek64(fd,offset,whence);
    if (hoff != foff) {
...
int fseek(FILE *stream, long offset, int whence) {
    ...
    int hoff=hdf5_lseek(fd,offset,whence);
#ifdef BOTH_HDF_AND_FILE
    int foff=__real_fseek(stream,offset,whence);
    if (hoff != foff) {
        LOG_WARN("difference between hdf/file in '%s': %d/%d",
                filename_table+fd*PATH_MAX,hoff,foff);
    }
...
int __fxstat64 (int __ver, int fd, struct stat64 *buf) {
    if (! handle_table[fd])
        return(__real___fxstat64(__ver,fd,buf));
    LOG_DBG2("'%s'", filename_table+fd*PATH_MAX);
